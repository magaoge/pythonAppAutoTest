...
from tools.path_maneger import get_screen_shot_path
...
class Basic:
    ...
    # 判断页面是否存在元素
    def wait_element_Presence(self,locator,timeout = 15,poll_frequency=0.5,doc=""):
        ...
        try:
            ...
            #将error级别信息,同时显示在控制台
            MyLog.exception("{}元素显示失败!".format(doc))
            self.my_save_screenshot(doc)
            raise
    ...
    def get_element(self,locator,doc=""):
        ...
        try:
            # 这里考虑到两点：
            # 1.外部类调用的时候，使用者可能不知道底层在寻找元素时封装了等到元素加载，进行重复的代码编写；
            # 2.不够灵活，可能有些元素需要的等待机制是可见，或者其他之类的；
            # 总结：总之不够友好
...
#     driver.save_screenshot(file_path)
    ...
    # 右滑动
    def swipe_right(self,element="",doc=""):
        MyLog.info("{}========操作屏幕右滑".format(doc))
        try:

            windowSize = self.get_window_size(element = element,doc=doc)

            small_x = windowSize['width'] * 0.1
            y = windowSize['height'] * 0.5
            big_x = windowSize['width'] * 0.9

            self.driver.swipe(big_x,y,small_x,y,1000)
        except:
            MyLog.exception("{}========操作屏幕右滑失败".format(doc))
            self.my_save_screenshot(doc)
            raise

    # 左滑动
    def swipe_left(self,element="",doc=""):
        MyLog.info("{}========操作屏幕左滑".format(doc))
        try:
            windowSize = self.get_window_size(element = element,doc=doc)
            ...
            self.driver.swipe(small_x,y,big_x,y,1000)
        except:
            MyLog.exception("{}========操作屏幕左滑失败".format(doc))
            self.my_save_screenshot(doc)
            raise

    # 上滑动
    def swipe_up(self,element="",doc=""):
        MyLog.info("{}========操作屏幕上滑".format(doc))
        try:
            windowSize = self.get_window_size(element = element,doc=doc)

            small_y = windowSize['height'] * 0.1
            x = windowSize['width'] * 0.5
            big_y = windowSize['height'] * 0.9

            self.driver.swipe(x, big_y, x, small_y, 1000)
        except:
            MyLog.exception("{}========操作屏幕上滑失败".format(doc))
            self.my_save_screenshot(doc)
            raise

    # 下滑动
    def swipe_down(self,element="",doc=""):
        MyLog.info("{}========操作屏幕下滑".format(doc))
        try:
            windowSize = self.get_window_size(element = element,doc=doc)
            ...
            self.driver.swipe(x, small_y, x, big_y, 1000)
        except:
            MyLog.exception("{}========操作屏幕下滑失败".format(doc))
            self.my_save_screenshot(doc)
            raise

    # 获取窗口大小
    def get_window_size(self,element="",doc=""):
        MyLog.info("{}获取元素窗口大小".format(doc))
        try:
            if element != "":
                ele = self.get_element(element,"获取元素窗口大小")
                windowSize = ele.size
            else:
                windowSize = self.driver.get_window_size()
            return windowSize
        except:
            MyLog.exception("{}获取元素窗口大小失败".format(doc))
            self.my_save_screenshot(doc)
            raise

    # toast窗口获取
    # 注意一定要打开appuim中的automationName = UIautomator2
    def get_toastMsg(self,toastTXT,doc=""):
        MyLog.info("{}获取元素toast元素文本内容{}".format(doc,toastTXT))
        try:
            loc = "//*[contains(@text,'{}')]".format(toastTXT)
            toastMsg = self.get_element_text(loc)
            return toastMsg
        except:
            MyLog.info("{}获取元素toast元素文本内容{}失败".format(doc,toastTXT))
            self.my_save_screenshot(doc)
            raise
    # 点击，可多点点击
    def tap_point(self,point,duration = 500):
        r"""
        :param point: List[Tuple[int0, int1]],int0 = x, int1 = y
        :param duration: 点触时间，单位毫秒
        :return:

        待优化：其实对于外层引用的人，这个方式并不友好，因为不论引用还是阅读，开发人员都不太清楚，哪个是x,哪个是y
        后续可以考虑，改造传入的数格式为列表嵌套字典的格式
        """
        self.driver.tap(point,duration)
    ...
    # 进入当前上下文（context可以理解为窗口）
    def switch_context(self,action = None,doc=""):
        r"""
        :param action: 返回默认APP上下文的action的参数是default
        :return:
        """
        MyLog.info("{}context上下文切换".format(doc))
        ...
        try:
            if action is None :
                ...
                print(context_list)
                # 获取当前的上下文句柄，但是也没有到当前打开的句柄页面，不知道为什么？
                current_context = self.driver.current_context
                print(current_context)
                # 遍历当前上下文句柄
                for context in context_list:
                    if context == context_list[-1]:
                        self.driver.switch_to.context(context)
                        break
            elif action == "default":
                # 目前不生效，暂时认定是因为webdriver不匹配导致不生效
                self.driver.switch_to.context(None)

            else:
                MyLog.info("{}未知的switch_context上下文操作".format(doc))
                print("未知的switch_context上下文操作")
        except:
            MyLog.info("{}switch_context上下文操作失败".format(doc))
            self.my_save_screenshot(doc)
            raise

    # 触屏操作(仅提供思路，可以将点从外部传入，也可以直接写死，因为其实滑动的手势其实也就只有那么几种)
    # def touch_point(self):
    #     x = [(1,2),(3,4)]
    #     m = ""
    #     for i in x:
    #         m = TouchAction(self.driver).press(*i).wait(200).perform()
    #     m.release().perform()

from PageLocator.page_login import LoginPageElement as loc

# if __name__ == '__main__':
#     Basic().click_element(loc.skip_guide_button)
